...
{ "properties": { "magnitude": 800 }, "geometry": { "type": "Feature", "coordinates": [ 114.1736, 22.2950],"type":"Point" } }
      ...
      const Heatmap = ({ data }) => {
        ...
        const [refreshInterval, setRefreshInterval] = useState(15000*60);
  ...
  // Calculate arrow coordinates based on marker location and direction
    ...
    <span class="color-box" style={{backgroundColor: '#501E49'}}></span><span class="color-label">{categories[1]+1} - {categories[0]}</span>
      ...
      <nav id="filter-group" class="filter-group" style={{position: 'absolute'}}>      
      <select id="map-style-select" value={refreshInterval} onChange={e => setRefreshInterval(e.target.value)}>
      <option value="15000">15s</option>
      <option value="10000">30s</option>
    </select></nav><p>Countdown: {countdown}</p>
    </>);
  };
